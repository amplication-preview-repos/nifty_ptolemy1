/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DataEmbeddingWhereInput } from "./DataEmbeddingWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { DataEmbeddingOrderByInput } from "./DataEmbeddingOrderByInput";

@ArgsType()
class DataEmbeddingFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DataEmbeddingWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => DataEmbeddingWhereInput, { nullable: true })
  @Type(() => DataEmbeddingWhereInput)
  where?: DataEmbeddingWhereInput;

  @ApiProperty({
    required: false,
    type: [DataEmbeddingOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [DataEmbeddingOrderByInput], { nullable: true })
  @Type(() => DataEmbeddingOrderByInput)
  orderBy?: Array<DataEmbeddingOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DataEmbeddingFindManyArgs as DataEmbeddingFindManyArgs };
